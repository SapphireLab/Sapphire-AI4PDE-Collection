...
- $\bm y := (\bm x, t) = (x_1, \cdots, x_{d-1}, t)^{\mathsf{T}}\in \mathbb{R}^d$ 是包含空间和时间变量的列向量;
- $u$ 是偏微分方程的未知解;
...
$$
  \mathcal{P}_{\rm NN} = {span}\Big\{1, \sigma(\bm w_1^{\mathsf{T}} \bm y + b_1), \cdots, \sigma(\bm w_M^{\mathsf{T}} \bm y + b_M) \Big\},\tag{03}
$$
...
我们可以将每个隐藏神经元 $\sigma(\bm w_m^{\mathsf{T}} \bm y + b_m)$ 视为从 $\bm y \in \mathbb{R}^{d}$ 空间到输出空间 $\mathbb{R}$ 的非线性特征映射.
从逼近论的角度看, 隐藏神经元集合 $\{\sigma(\bm w_m^{\mathsf{T}} \bm y + b_m)\}_{m=1}^M$ 可以视为 $\mathbb{R}^d$ 空间中的全局支持基.

**神经特征空间** $\mathcal{P}_{\rm NN}$ 可以定义为由基 $\{\sigma(\bm w_m^{\mathsf{T}} \bm y + b_m)\}_{m=1}^M$ 张成的线性空间, 即:
...
那么, 前面定义的单隐层全连接神经网络就落在这个线性空间中, 即 $u_{\rm NN}(\bm y) \in \mathcal{P}_{\rm NN}$.
换句话说, 神经网络近似可以视为使用非正交基的谱方法, 输出层的参数 $\bm \alpha$ 就是在神经特征空间 $\mathcal{P}_{\rm NN}$ 中的展开系数.

在[物理信息神经网络方法](PINN.md)中, 神经特征空间 $\mathcal{P}_{\rm NN}$ 和系数 $\bm \alpha$ 都是通过随机梯度下降方法同时训练的, 这往往导致非凸且病态优化问题.
神经网络训练中的非凸性和病态性被发现是得到不满意精度的主要原因.

一个自然的想法就是减少训练的复杂性, 将 $\mathcal{P}_{\rm NN}$ 的训练与 $\bm \alpha$ 的训练进行解耦.
例如, 在随机特征模型中, $\mathcal{P}_{\rm NN}$ 是通过从用户预定义的分布中随机生成的 $\{(\bm w_m, b_m)\}_{m=1}^M$ 来定义的, 而系数 $\bm \alpha$ 可以通过解线性系统来获得, 此时的 $\mathcal{L}$, $\mathcal{B}$ 均为线性算子.
...
构造神经特征空间的第一步是重新参数化每个隐藏神经元 $\sigma(\bm w_m^{\mathsf{T}} \bm y + b_m)$ 的权重和偏置为确定神经元位置和形状的不同参数.
每个神经元可以视为定义域 $\Omega$ 中的全局支持基函数 $\psi_{m}(\bm y):=\sigma(\bm w_m^{\mathsf{T}} \bm y + b_m)$.
...
我们就获得了 $(\bm w_m, b_m)$ 的重新参数化, 即位置参数 $(\bm a_m, r_m)$ 和形状参数 $\gamma_m$.
...
通过重新参数化, 现在每个参数对于基函数 $\psi_{m}(\bm y)$ 都有不同的几何意义.
...
更具体地, 单位向量 $\bm a_m$ 表示划分超平面的法方向, $|r_m|$ 则表示其距离原点的距离.
形状参数 $\gamma_m$ 控制着 $\bm w_m^{\mathsf{T}}\bm y + b_m$ 在法方向 $\bm a_m$ 上的陡峭程度, 即基函数 $\psi_m(\bm y)$ 从 -1 到 1 的过渡率.
...
图 01 展示了三个二维基函数, 它们具有不同的形状和位置参数, 其中黑色虚线表示划分超平面, 黑色箭头表示法方向 $\bm a_m$ 缩放 $|r_m|\text{sign}(\gamma_m)$ 倍.
箭头标记了 $\psi_m(\bm y)$ 的增长方向, 其长度等于从原点到超平面的距离.
