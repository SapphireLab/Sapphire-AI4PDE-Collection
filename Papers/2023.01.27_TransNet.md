...
> A typical method [36] for solving the PDE in Eq.01 is to directly parameterize the solution $u(\bm y)$ as a neural network $u_{\rm NN}(\bm y)$ in Eq.02 and optimize the neural network's parameters by minimizing the PDE residual loss, e.g., $L(\bm y) = \|\mathcal{L}(u(\bm y)) - \mathcal{L}(u_{\rm NN}(\bm y))\|_2 + \|\mathcal{B}(u(\bm y)) - \mathcal{B}(u_{\rm NN}(\bm y)) \|_2$, at a set of spatial-temporal locations. 
...
> In comparison, the random feature models require much lower re-training costs, which has been exploited in learning operators [33] and dynamical systems [19]; [37].
