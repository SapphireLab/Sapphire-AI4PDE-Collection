...
>
> Random feature models together with shallow neural networks have also been used to solve PDEs ([18]; [19](Random-Feature-Interacting-System.md); [20]) or learn PDE operators [33](Random-Features-Banach.md). 
...
\left\{
\begin{aligned}
  & \mathcal{L}(u(y)) = f(y)\quad  \text{ for }\quad y \in \Omega,\\
  & \mathcal{B}(u(y)) = g(y)\quad  \text{ for }\quad y \in \partial \Omega,
\end{aligned}
\right.\tag{01}
$$

> where $\Omega \subset \mathbb{R}^{d}$ with the boundary $\partial \Omega$ is the spatial-temporal bounded domain under consideration, $ y := (\textbf{x}, t) = (x_1, \cdots, x_{d-1}, t)^{\mathsf{T}}\in \mathbb{R}^d$ is a column vector includes both spatial and temporal variables, $u$ denotes the unknown solution of the PDE, $\mathcal{L}(\cdot)$ is a differential operator, $\mathcal{B}(\cdot)$ is the operator defining the initial and/or boundary conditions, $ f(y)$ and $g(y)$ are the right hand sides associated with the operators $\mathcal{L}(\cdot)$ and $\mathcal{B}(\cdot)$, respectively. 
...
>
> $$
>   u_{\rm NN}(y) := \sum_{m = 1}^M \alpha_m\, \sigma( w_m^{\mathsf{T}}  y + b_m) + \alpha_0,\tag{02}
> $$
>
> where $M$ is the number of hidden neurons, the vector $w_m = (w_{m,1}, \cdots, w_{m,d})^{\mathsf{T}}$ and the scalar $b_m$ are the weights and bias of the $m$-th hidden neuron, the vector $\alpha = (\alpha_0,\alpha_1,\cdots,\alpha_M)^{\mathsf{T}}$ includes the weights and bias of the output layer, and $\sigma(\cdot)$ is the activation function. 
...
> [A typical method [36]](PIML.md) for solving the PDE in Eq.01 is to directly parameterize the solution $u(y)$ as a neural network $u_{\rm NN}(y)$ in Eq.02 and optimize the neural network's parameters by minimizing the PDE residual loss, e.g., $L(y) = \|\mathcal{L}(u(y)) - \mathcal{L}(u_{\rm NN}(y))\|_2 + \|\mathcal{B}(u(y)) - \mathcal{B}(u_{\rm NN}(y)) \|_2$, at a set of spatial-temporal locations. 
...
> When there is any change to the operators $\mathcal{L}(\cdot), \mathcal{B}(\cdot)$, the right-hand-side functions $f(y), g(y)$, or the shape of the domain $\Omega$, the neural network $u_{\rm NN}(y)$ often needs to be re-trained using gradient-based optimization (even though the current parameter values could provide a good initial guess for the re-training), or the hyperparameters associated with the network and the optimizer need to be re-tuned. 
...
- $ y := (\textbf{x}, t) = (x_1, \cdots, x_{d-1}, t)^{\mathsf{T}}\in \mathbb{R}^d$ 是包含空间和时间变量的列向量;
- $u$ 是偏微分方程的未知解;
- $\mathcal{L}(\cdot)$ 是微分算子;
- $\mathcal{B}(\cdot)$ 是定义初始条件和/或边界条件的算子;
- $f(y)$ 是与 $\mathcal{L}(\cdot)$ 相关的右端项.
- $g(y)$ 是与 $\mathcal{B}(\cdot)$ 相关的右端项.
...
其中:
- $M$ 是隐层神经元的数量;
- $w_m = (w_{m,1}, \cdots, w_{m,d})^{\mathsf{T}}$ 是第 $m$ 个隐层神经元的权重向量;
- $b_m$ 是第 $m$ 个隐层神经元的偏置;
- $\alpha = (\alpha_0,\alpha_1,\cdots,\alpha_M)^{\mathsf{T}}$ 是输出层的权重向量;
- $\sigma(\cdot)$ 是激活函数.
...
[Physics-Informed Machine Learning (PIML) [36]](PIML.md) 中指出求解上述偏微分方程的一种典型方法是直接将解 $u(y)$ 参数化为形如上述的神经网络 $u_{\rm NN}(y)$, 并通过最小化在时空样本点集上的偏微分方程残差损失 $L(y) = \|\mathcal{L}(u(y)) - \mathcal{L}(u_{\rm NN}(y))\|_2 + \|\mathcal{B}(u(y)) - \mathcal{B}(u_{\rm NN}(y)) \|_2$ 来优化神经网络的参数.

尽管这些方法在求解偏微分方程问题上有不错的性能, 但其主要的缺点是**有限的可迁移性**, 这是因为这类方法遇到新的问题时需要采用基于梯度的重新训练和超参数重新调整, 这使得其计算代价高昂.
当对算子, 右端项函数或定义域形状有任何修改时, 神经网络 $u_{\rm NN}(y)$ 通常需要通过基于梯度的优化方法重新训练 (即便当前参数值能够为重新训练提供一个良好的初始值), 或者对网络和优化器相关的超参数进行重新调整.
...
>
> We can treat each hidden neuron $\sigma(w_m^{\mathsf{T}}  y + b_m)$ as a nonlinear feature map from the space of $ y \in \mathbb{R}^{d}$ to the output space $\mathbb{R}$. 
> From the perspective of approximation theory, the set of hidden neurons $\{\sigma(w_m^{\mathsf{T}}  y + b_m)\}_{m=1}^M$ can be viewed as a globally supported basis in $\mathbb{R}^d$. 
> The **neural feature space**, denoted by $\mathcal{P}_{\rm NN}$, can be defined by the linear space expanded by the basis $\{\sigma(w_m^{\mathsf{T}}  y + b_m)\}_{m=1}^M$, i.e., 

$$
  \mathcal{P}_{\rm NN} = {span}\left\{1, \sigma(w_1^{\mathsf{T}}  y + b_1), \cdots, \sigma(w_m^{\mathsf{T}}  y + b_M) \right\},\tag{03}
$$

> where the constant basis corresponds to the bias of the output layer. 
> Then, the neural network in Eq.02 lives in the linear space, i.e., $u_{\rm NN}(y) \in \mathcal{P}_{\rm NN}.$
> In other words, the neural network approximation can be viewed as a spectral method with **non-orthogonal** basis, and the parameters $\alpha$ in Eq.02 of the output layer of $u_{\rm NN}(y)$ is the coefficients of the expansion in the neural feature space $\mathcal{P}_{\rm NN}$. 
>
> In the PINN methods, the neural feature space $\mathcal{P}_{\rm NN}$ and the coefficient $\alpha$ are trained simultaneously using SGD methods, which often leads to a non-convex and ill-conditioned optimization problem. 
...
> A natural idea to reduce the complexity of the training is to decouple the training of $\mathcal{P}_{\rm NN}$ from that of $\alpha$. 
> For example, in random feature models, $\mathcal{P}_{\rm NN}$ is defined by randomly generating the parameters $\{(w_m, b_m)\}_{m=1}^M$ from a user-defined probability distribution; the coefficients $\alpha$ can then be obtained by solving a linear system when the operators $\mathcal{L}$, $\mathcal{B}$ in Eq.01 are linear. 
...
我们可以将每个隐藏神经元 $\sigma(w_m^{\mathsf{T}}  y + b_m)$ 视为从 $ y \in \mathbb{R}^{d}$ 空间到输出空间 $\mathbb{R}$ 的非线性特征映射.
从逼近论的角度看, 隐藏神经元集合 $\{\sigma(w_m^{\mathsf{T}}  y + b_m)\}_{m=1}^M$ 可以视为 $\mathbb{R}^d$ 空间中的全局支持基.

**神经特征空间** $\mathcal{P}_{\rm NN}$ 可以定义为由基 $\{\sigma(w_m^{\mathsf{T}}  y + b_m)\}_{m=1}^M$ 张成的线性空间, 即:

$$
  \mathcal{P}_{\rm NN} = {span}\Big\{1, \sigma(w_1^{\mathsf{T}}  y + b_1), \cdots, \sigma(w_m^{\mathsf{T}}  y + b_M) \Big\},\tag{03}
$$

其中常数基对应于输出层的偏置.

那么, 前面定义的单隐层全连接神经网络就落在这个线性空间中, 即 $u_{\rm NN}(y) \in \mathcal{P}_{\rm NN}$.
换句话说, 神经网络近似可以视为使用非正交基的谱方法, 输出层的参数 $\alpha$ 就是在神经特征空间 $\mathcal{P}_{\rm NN}$ 中的展开系数.

在[物理信息神经网络方法](PINN.md)中, 神经特征空间 $\mathcal{P}_{\rm NN}$ 和系数 $\alpha$ 都是通过随机梯度下降方法同时训练的, 这往往导致非凸且病态优化问题.
神经网络训练中的非凸性和病态性被发现是得到不满意精度的主要原因.

一个自然的想法就是减少训练的复杂性, 将 $\mathcal{P}_{\rm NN}$ 的训练与 $\alpha$ 的训练进行解耦.
例如, 在随机特征模型中, $\mathcal{P}_{\rm NN}$ 是通过从用户预定义的分布中随机生成的 $\{(w_m, b_m)\}_{m=1}^M$ 来定义的, 而系数 $\alpha$ 可以通过解线性系统来获得, 此时的 $\mathcal{L}$, $\mathcal{B}$ 均为线性算子.

然而, [实验一节](#experiments)的结果表明基于单隐层全连接神经网络的随机特征模型随特征数量增加而收敛缓慢.
这一缺点激励我们建立一种方法来自定义神经特征空间来提升求解偏微分方程时的精度, 效率和可迁移性.
...
>
> The first step is to re-parameterize the weight and bias of each hidden neuron $\sigma(w_m^{\mathsf{T}}  y + b_m)$, into distinct parameters that determine the location and shape of the neuron.
> Each hidden neuron is viewed as a globally supported basis function $\psi_{m}(y)$ in $\Omega$, i.e., $\psi_{m}(y)=\sigma(w_m^{\mathsf{T}}  y + b_m)$.
> If we rewrite the hidden neuron as

$$
  \sigma(w_m^{\mathsf{T}}  y + b_m)=\sigma(\gamma_m(a_m^{\mathsf{T}}  y + r_m)),\tag{04}
$$

> where $\|a_m\|_2 = 1$ and $\gamma_m\geq 0$, we obtain the re-parameterization of $(w_m, b_m)$ into the location parameter $(a_m, r_m)$ and shape parameter $\gamma_m$. 
...
$$
  ...
  \Leftrightarrow
  \begin{cases}
    a_m = \dfrac{w_m}{\|w_m\|_2} \\
    r_m = \dfrac{b_m}{\|w_m\|_2} \\
    \gamma_m = \|w_m\|_2
  \end{cases}\tag{05}
$$

> After the re-parameterization, each parameter now holds a distinct geometric interpretation towards the basis function $\psi_{m}(y)$. 
>
...
> More specifically, the unit vector $a_m$ represents the normal direction of the partition hyperplane, while $|r_m|$ indicates its distance from the origin.
> The shape parameter $\gamma_m$ governs the steepness of the pre-activation value $w_m^{\mathsf{T}} y + b_m$ along the normal direction $a_m$, which translates to the transition rate from -1 to 1 on basis $\psi_m(y)$. 
> This can also be geometrically interpreted as the width of the transition band (see [Figure.01](#figure.01)). 
>
> Figure.01 shows three 2D basis functions with different shape and location parameters, where the black dashed lines represent the partition hyperplanes and the black arrows denote the normal direction $a_m$ scaled by $|r_m|\text{sign}(\gamma_m)$, i.e., $a_m|r_m|\text{sign}(\gamma_m)$. 
> The arrow marks the direction of increase in $\psi_m(y)$, and its length equals the distance from the origin to the hyperplane. 
...
构造神经特征空间的第一步是重新参数化每个隐藏神经元 $\sigma(w_m^{\mathsf{T}}  y + b_m)$ 的权重和偏置为确定神经元位置和形状的不同参数.
每个神经元可以视为定义域 $\Omega$ 中的全局支持基函数 $\psi_{m}(y):=\sigma(w_m^{\mathsf{T}}  y + b_m)$.
...
我们就获得了 $(w_m, b_m)$ 的重新参数化, 即位置参数 $(a_m, r_m)$ 和形状参数 $\gamma_m$.
...
通过重新参数化, 现在每个参数对于基函数 $\psi_{m}(y)$ 都有不同的几何意义.

---

当激活函数 $\sigma$ 为 ReLU 时, 有一个重要的概念叫做**划分超平面**, 它将激活区域和未激活区域分开, 定义为:

$$
  w_m^{\mathsf{T}}  y + b_m = 0\tag{06}.
$$

这一划分超平面可以通过位置参数精确重写为:

$$
  a_m^{\mathsf{T}} y + r_m = 0\tag{07}.
$$

划分超平面的几何意义对于其他激活函数也同样重要.

例如 $tanh(\cdot)$ 由于其平滑性被广泛用于求解偏微分方程.
我们现在讨论激活函数为 $tanh(\cdot)$ 时, 形状参数和位置参数的几何解释:
此时的划分超平面描述了从 -1 到 1 的过渡平面.
更具体地, 单位向量 $a_m$ 表示划分超平面的法方向, $|r_m|$ 则表示其距离原点的距离.
形状参数 $\gamma_m$ 控制着 $w_m^{\mathsf{T}} y + b_m$ 在法方向 $a_m$ 上的陡峭程度, 即基函数 $\psi_m(y)$ 从 -1 到 1 的过渡率.
这还可以用过渡带的宽度来进行几何解释, 见 [图 01](#figure.01).

---

![<a id="figure.01"></a>](../Images/2023.01.27_TransNet/Fig.01.png)

图 01. 对方程 04 中定义的具有不同位置和形状参数的神经基函数 $\psi_m(y)$ 的可视化.
黑色虚线表示划分超平面, 黑色箭头 $a_m |r_m|\text{sign}(\gamma_m)$ 是对法方向 $a_m$ 进行缩放 $|r_m|\text{sign}(\gamma_m)$ 倍.

---

图 01 展示了三个二维基函数, 它们具有不同的形状和位置参数, 其中黑色虚线表示划分超平面, 黑色箭头表示法方向 $a_m$ 缩放 $|r_m|\text{sign}(\gamma_m)$ 倍.
箭头标记了 $\psi_m(y)$ 的增长方向, 其长度等于从原点到超平面的距离.
显然, 基函数在划分超平面的值在沿着法方向改变, 而在垂直于法方向上保持恒定.
对于大 $\gamma$ 值对应的过渡带比小 $\gamma$ 的要窄.

比较不同的基函数 (中到左, 中到右), 可以看出改变位置参数仅仅影响基函数的位置而不影响形状, 反之亦然.

因此, 重参数化技巧有效地分离了位置和形状参数, 每个参数都具有几乎独立的几何意义, 这使得我们能够更自然地以不同方式处理它们.
...
>
> The second step for constructing $\mathcal{P}_{\rm NN}$ is to determine the location parameters $\{(a_m,r_m)\}^{M}_{m=1}$ in Eq.04. 
...
> And our approach is to generate a set of random $\{(a_m,r_m)\}^{M}_{m=1}$, to ensure the creation of uniformly distributed partition hyperplanes within $\Omega$.
>
> The idea for handling the location parameters for $\psi_m(y)$ through the partition hyperplane is inspired by studies on the activation patterns of ReLU networks. 
...
>
> In this subsection, we assume $\Omega$ is a unit ball, i.e., $B_1(\textbf{0}) = \{y : \|y\|_2\leq 1\} \subset \mathbb{R}^d$. 
> For an arbitrary point $y\in\Omega$, we denote the partition hyperplane density as $D_M^{\tau}(y)$, where the $M$ is the number of partition hyperplanes and $\tau > 0$ is the bandwidth for calculating the density.
> To proceed, we first measure the distance from $ y$ to each partition hyperplane defined in Eq.07 by
> $$
> d_m(y) = \frac{\|a_m^{\mathsf{T}} y + r_m\|_2}{\|a_m\|_2} = |a_m^{\mathsf{T}} y + r_m |, \quad m=1,\cdots,M.\tag{08}
> $$
>
> Based on the above point-to-plane distance, we then define the partition hyperplane density $D_M^{\tau}(y)$ as
> $$
> D_m^{\tau}(y) = \dfrac{1}{M}\sum_{m=1}^{M} \textbf{1}_{\{d_m(y)< \tau\}}(y),\tag{09}
> $$
> 
> where $\textbf{1}_{\{d_m(y)< \tau\}}(y)$ is the indicator function of whether the distance between $ y$ and the $m$-th partition hyperplane is smaller than $\tau$. 
> Intuitively, $D_M^{\tau}(y)$ measures the percentage of neurons whose partition hyperplane intersects the ball with center $ y$ and radius $\tau$, $B_{\tau}(y)$.
> The uniformity of the partition hyperplane can be formally measured by how much $D_M(y)$ varies in $\Omega$.
>
> Next, we outline the sampling process for the random location parameters $\{(a_m,r_m)\}^{M}_{m=1}$ that yield a constant hyperplane density $D_M^{\tau}(y)$ in $\Omega$ under expectation. 
> Specifically, we start by generating the normal directions $\{a_m\}^M_{m=1}$ uniformly distributed on the $d$-dimensional unit sphere. 
...
> To address this, we draw samples from the d-dimensional standard Gaussian distribution in the Cartesian coordinate system and normalize the samples to unit vectors to obtain $\{a_m\}^M_{m=1}$. 
> Then, $\{r_m\}^{M}_{m=1}$ is generated from a uniform distribution on $[0, 1]$. 
> The complete sampling procedure is expressed as follows:
> $$
> a_m = \dfrac{X_m}{\| X_m \|_2},\quad r_m = U_m,\quad m=1,\cdots,M,\tag{10}
> $$
>
> where $X_m$ are i.i.d. standard Gaussian distribution and $U_m$ follows i.i.d. uniform distribution on $[0, 1]$. 
> The subsequent theorem demonstrates that our approach yields a set of uniformly distributed partition hyperplanes in $\Omega$, measured by a constant expected density $\mathbb{E}[D_M^{\tau}(y)]$ in $\Omega$.
...
> $$
> a_m^{\mathsf{T}} y + r_m = 0
> $$
> if $\{a_m\}^M_{m=1}$ are i.i.d. and uniformly distributed on the $d$-dimensional unit sphere, i.e., $\| a_m \|_2 = 1$, and $\{r_m\}^M_{m=1}$ are i.i.d. and uniformly distributed in $[0, 1]$, then, for a fixed $\tau\in (0, 1)$,
> $$
>  \mathbb{E}[D_M^{\tau}(y)] = \tau,\quad  \forall \|  y \|_2 \leq  1 − \tau,
> $$
> 
> where $D_m^{\tau}(y)$ is the density function defined as follow (Eq.09).
>
> $$
>  D_m^{\tau}(y) = \dfrac{1}{M}\sum_{m=1}^{M} \textbf{1}_{\{d_m(y)< \tau\}}(y)
> $$
>
> ---
> **Proof**
>
> For a fixed $ y\in\Omega$, we can write the expectation of $D_M^{\tau}(y)$ as
> $$
> \begin{aligned}
> \mathbb{E}[D_M^{\tau}(y)] &= \mathbb{E}[\dfrac{1}{M}\sum_{m=1}^{M} \textbf{1}_{\{d_m(y)< \tau\}}(y)] \\
>  &=\dfrac{1}{M}\sum_{m=1}^{M} \mathbb{E}[\textbf{1}_{\{d_m(y)< \tau\}}(y)] \\
> \end{aligned}\tag{11~12}
> $$
>
> Use the i.i.d property of $\{a_m\}_{m=1}^M$ and $\{r_m\}_{m=1}^M$, we have
> $$
> \mathbb{E}[\textbf{1}_{\{d_m(y)< \tau\}}(y)] = \mathbb{E}[\textbf{1}_{\{d_{m'}(y)< \tau\}}(y)],\quad \forall 1\leq m, m' \leq M.\tag{13}
> $$
>
> Therefore, we can simplify the notation by dropping the subscript $m$ from $(a_m, r_m)$ and $\textbf{1}_{\{d_m(y)< \tau\}}(y)$, and use $(a, r)$ and $\textbf{1}_{\{d(y)< \tau\}}(y)$ to denote them in the following derivation.
>
> The expected density can now be written as
> $$
> \begin{aligned}
> \mathbb{E}[D_M^{\tau}(y)] &= \mathbb{E}[\textbf{1}_{\{d(y)< \tau\}}(y)] \\
> &= \mathbb{E}[\textbf{1}_{\{|a^{\mathsf{T}} y+r|< \tau\}}(y)] \\
> &= \mathbb{P}[|a^{\mathsf{T}} y + r|<\tau] 
> \end{aligned}\tag{14}
> $$
>
> Given the independence between $a$ and $r$, and the fact that the distribution of $r$ is known, we are only left with the unknown distribution of $a^{\mathsf{T}}  y$.
> By our assumption, $a$ is uniformly distributed on a $d$-dimensional unit sphere, and $ y$ is a constant vector.
>
> To proceed, we begin by considering a unitary matrix $Q$, such as $Q y = (\|  y\|_2,0,\cdots,0)^{\mathsf{T}}$ and define $\tilde{ y} = Q y$ and $\tilde{a} = Qa$.
> And we can write the inner product as follows:
> $$
> a^{\mathsf{T}} y = a^{\mathsf{T}} (Q^{\mathsf{T}}Q) y=(Qa)^{\mathsf{T}}(Q y) = \tilde{a}^{\mathsf{T}}\tilde{ y} = \tilde{a}_1 \|  y\|_2
> $$
>
> where $\tilde{a}_1$ is the first component of $\tilde{a}$.
> Now we only need to find the distribution of $\tilde{a}_1$.
>
> Given that $Q$ is a unitary (rotation) matrix and $a$ is uniform on the $d$-dimensional unit sphere, $\tilde{a}$ will also follow the same distribution as $a$.
> By the construction of $a$ in Eq.10, we know $\tilde{a}$ can also be written in the same form as:
> $$
> \tilde{a} =\dfrac{X}{\| X\|_2}
> $$
>
...
>
> Next we derive the analytical form of the probability $\mathbb{P}[|a^{\mathsf{T}} y + r|<\tau]$.
> With the probability density function $p_{\tilde{a}_1}(z)$, we can write the probability as the following integral
> $$
> \begin{aligned}
> \mathbb{P}[|a^{\mathsf{T}} y + r|<\tau]
> &= \mathbb{P}[|\tilde{a}_1 \|  y \|_2 + r| <\tau]\\
> &= \int_{\{s<-z \| y\|_2+\tau\}\cup \{s>-z\|  y \|_2 -\tau\}} p_{\tilde{a}_1}(z) p_r(s)\text{d}z\text{d}s\\
> &= \int_{\{-z\|  y\|_2 - \tau < s < -z\|  y\|_2 + \tau\}} p_{\tilde{a}_1}(z)\text{d}z\text{d}s.
> \end{aligned}
> $$
>
> Here the support for $z$ and $s$ are $[-1,1]$ and $[0,1]$ respectively.
> Assume that $\|  y\|_2 \leq 1-\tau$, then the above integral can be exactly calculated by the following two cases.
>
> **Cases 1: $\|  y\|_2<\tau$**. 
> In this case, the integration domain is prescribed by four lines: $s=-z\|  y\|_2+\tau$, $s=0$, $z=-1$, and $z=1$.
...
> $$
>
> **Case 2: $\tau \leq \|  y\|_2 \leq 1-\tau$**.
> In this case, the integration domain is defined by four lines: $s=-z \|  y\|_2+\tau$, $s=-z\|  y\|_2 - \tau$, and $s=0$, $z=-1$.
> Once again, utilizing the symmetry property $p_{\tilde{a}_1}(z)$, we can redefine the integration domain as bounded by: $s=-z\|  y\|_2+\tau$, $s=-z\|  y\|_2 - \tau$, $z=0$ and $z=-1$. 
> The integral is calculated as 
> 
> $$
> \begin{aligned}
> \mathbb{P}[|a^{\mathsf{T}} y + r|<\tau]
> &= \int_{-1}^{0}\int_{-z\|  y\|_2 - tau}^{-z\|  y\|_2 + tau} p_{\tilde{a}_1}(z)\text{d}s\text{d}z\\
...
> $$
> \mathbb{P} [|a^{\mathsf{T}} y + r|<\tau] = \tau\quad \text{for any} \|  y\|_2 \leq 1-\tau.
> $$
>
...
在高斯随机特征模型中, $w_m$ and $b_m$ 直接从独立同分布的标准高斯分布直接生成, 且所有划分超平面密度值 $D_{M}^{\tau}(y)$ 直接由蒙特卡洛模拟 ($M=50,000, \tau=0.1$) 估计得到.
...
>
> The third step is to tune the shape parameters $\{\gamma_m\}^M_{m=1}$ in Eq.04 that control the shape of the neural basis function $\psi_m(y)$. 
...
> 
> Let us use $G(y|\omega, \eta)$ to denote the GRF for generating the auxiliary tuning function, where $\omega$ represents the abstract randomness and $\eta$ is a fixed correlation length. 
> We first generate $K$ realizations of the GRF, denoted by $\{G(y|\omega_k, \eta)\}^{K}_{k=1}$. 
> For each realization, we evaluate its value $g^k_j= G(y_j|\omega_k, \eta)$ at $J$ samples that are $\{ y^k_j\}^J_{j=1}$ uniformly distributed in the unit ball $B_1(\textbf{0})$. 
...
> $$
> \begin{aligned}
> MSE_k(\gamma) &= \min_{\alpha} \sum_{j=1}^{J} [g_j^k - (\sum_{m=1}^M \alpha_m \sigma(\gamma a_m  y_j^k + \gamma r_m))]\\
> &= \min_{\alpha} \sum_{j=1}^{J} [g_j^k - (\sum_{m=1}^M \alpha_m \psi_m(y_j^k))]^2.
> \end{aligned}\tag{16}
> $$
>
> where $a_m$ and $r_m$ are generated according to Eq.10 and $\psi_0(\cdot) = 1$.
...
> Even though $\mathcal{P}_{\rm NN}$ is defined on the unit ball, i.e., $B_1(\textbf{0})$, we can always place the (bounded) domain $\Omega$ for the target PDE in $B_1(\textbf{0})$ by simple translation and dilation. 
...
> Let $\{ y_{j_1}^{PDE}\}_{j_1=1}^{J_1}$ and $\{ y_{j_2}^{BD}\}_{j_2=1}^{J_2}$ denote the samples from $\Omega$ and $\partial \Omega$ respectively.
> The neuron basis functions $\{\psi_m(y)\}_{m=0}^M$, where $\psi_0(y) = 1$, are constructed from the location and shape parameter described in [Section.3.3.2](#Sec3.2.2) and [Section.3.3.3](#Sec3.3.3).
> The PDE problem in Eq.01 can be written as an optimization problem as
> $$
> \min_{\alpha} 
> \left \{
> \dfrac{1}{J_1} \sum_{j_1=1}^{J_1} [f(y_{j_1}^{PDE}) - \mathcal{L}(\sum_{m=0}^M\alpha_m \psi_m(y_{j_1}^{PDE}))]^2+\dfrac{1}{J_2} \sum_{j_2=1}^{J_2} [g(y_{j_2}^{BD}) - \mathcal{B}(\sum_{m=0}^M\alpha_m \psi_m(y_{j_2}^{BD}))]^2
> \right\}
> $$
>
> When $\mathcal{L}$ and $\mathcal{B}$ in Eq.01 are linear operators, the unknown parameters $\alpha = (\alpha_0, \alpha_1, \cdots, \alpha_M)$ in Eq.18 can be easily determined by solving the following least squares problem, i.e.,
> $$
> \min_{\alpha} 
> \left \{
> \dfrac{1}{J_1} \sum_{j_1=1}^{J_1} [f(y_{j_1}^{PDE}) - \sum_{m=0}^M\alpha_m \mathcal{L}(\psi_m(y_{j_1}^{PDE}))]^2+\dfrac{1}{J_2} \sum_{j_2=1}^{J_2} [g(y_{j_2}^{BD}) - \sum_{m=0}^M\alpha_m \mathcal{B}(\psi_m(y_{j_2}^{BD}))]^2
> \right\}
> $$
>
...
> $$
> \mathcal{L}(y) = \| \mathcal{L}(u(y))−\mathcal{L}(u_{\rm NN}(y))\|_2^2+\| \mathcal{B}(u(y))−\mathcal{B}(u_{\rm NN}(y))\|_2^2.
> $$ 
> 
...
> In this work, we do not rely on any measurement data of the solution $u(y)$ when using ***TransNet*** to solve PDEs, because the operators $\mathcal{L}$ and $\mathcal{B}$ in Eq.01 are sufficient to ensure the existence and uniqueness of the PDE's solution. 
> On the other hand, if any extra data of $u(y)$ are available, ***TransNet*** can easily incorporate it into the least squares problem in Eq.18 as a supervised learning loss.
...
> For the feature space for the two-dimensional PDEs, we sample each realization at $50^2$ uniformly distributed locations in $B_1(\textbf{0})$, i.e., $J = 2500$ in Eq.16, to compute the MSE in Eq.16. 
...
> Even though the purpose of ***TransNet*** is not to approximate the Gaussian process, it is interesting to check whether the uniform density $D_M(y)$ (proved in [Theorem.01](#theorem01)) leads to uniform error distribution. 
...
>
> [Figure.06](#figure.06) shows the density function $D_M(y)$ in Eq.09 of the feature spaces obtained by training PINN and the random feature models in solving the Poisson equation in the 2D space, i.e., case (C1) - (C4), where the constant $\tau$ in Eq.09 is set to $0.2$. 
...
> For example, since the set of neurons $\{\sigma(w_m^{\mathsf{T}} y + b_m)\}^M_{m=1}$ forms a non-orthogonal basis, it is possible to have linearly correlated neurons which will reduce the column rank of the least squares matrix, or even lead to an under-determined system. 
...
3. **特定最小二乘问题的性质**.
   ...
   例如, 由于神经元集合 $\{\sigma(w_m^{\mathsf{T}} y + b_m)\}^M_{m=1}$ 形成了一个非正交基, 因此可能存在线性相关的神经元, 这将导致最小二乘矩阵的列秩降低, 甚至导致一个欠定系统.
